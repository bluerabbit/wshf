...
//          $.print("file size->" + fileSize);
    ...
    fileOpen: function (fileName, iomode, block) {
      if (iomode == null) {
        iomode = FILE_IOMODE_FOR_READING;
      } else if (iomode == 'w') {
        iomode = FILE_IOMODE_FOR_WRITING; // 書込モード
      } else if (iomode == 'r') {
        iomode = FILE_IOMODE_FOR_READING; // 読取専用モード
      } else if (iomode == 'a') {
        iomode = FILE_IOMODE_FOR_APPENDING; // 追記モード
      }
    ...
    fileReadLine: function (fileName, block) {
      this.fileOpen(fileName, 'r', function (file) {
        var lineNo = 0;
        while(!file.AtEndOfStream) {
          block(file.ReadLine(), ++lineNo);
        }
      });
    },
    fileWriteLine: function (fileName, value) {
      this.fileOpen(fileName, 'a', function (textStream) {
        textStream.writeLine(value);
      });
    },
    ...
    fileGetBaseName: function (filename) {
      var file = this.fileSystemObject.GetFile(filename);
      return file.Name;
    },
    // Folder ------------------------------------------------------------------------------------------------
